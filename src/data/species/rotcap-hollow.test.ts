...
import {
  ROT_SOVEREIGN,
  ROT_SOVEREIGN_ADDS,
  ROTCAP_HOLLOW_AFFINITIES,
  ROTCAP_HOLLOW_BIOME,
  ROTCAP_HOLLOW_SPECIES_POOL,
  ROTCAP_HOLLOW_SPELLS,
} from './rotcap-hollow'

// Loader/shape test (per the H1/H2/H3 common checklist item 6): every creature has a valid
// affinity/species/rarity/trait/script reference; every trait/spell/status referenced actually
// exists in a registry.

const ALL_CREATURES = ROTCAP_HOLLOW_SPECIES_POOL.flatMap((s) => s.creatures)

describe('Rotcap Hollow: shape', () => {
  it('ships exactly 6 species x 3 creatures (18 total)', () => {
    expect(ROTCAP_HOLLOW_SPECIES_POOL).toHaveLength(6)
    for (const species of ROTCAP_HOLLOW_SPECIES_POOL) {
  ...
  it('every species has a unique id, every creature within it a unique id', () => {
    const speciesIds = new Set(ROTCAP_HOLLOW_SPECIES_POOL.map((s) => s.id))
  ...
  it('is affinity-complete (all 5 affinities present, "Coverage" note)', () => {
    const affinities = new Set(ROTCAP_HOLLOW_AFFINITIES)
  ...
  it('ROTCAP_HOLLOW_BIOME wires the real species pool', () => {
    expect(ROTCAP_HOLLOW_BIOME.speciesPool).toBe(ROTCAP_HOLLOW_SPECIES_POOL)
  })

  it('every ROTCAP_HOLLOW_SPELLS entry is tagged unlockedAtBiome:3 (this biome is the third tier)', () => {
    for (const spell of ROTCAP_HOLLOW_SPELLS) {
      expect(spell.unlockedAtBiome).toBe(3)
    }
  })
})

describe('Rot Sovereign (floor-30 boss)', () => {
  it('references a real registered trait', () => {
    expect(TRAIT_REGISTRY.get(ROT_SOVEREIGN_TRAIT.id)).toBe(ROT_SOVEREIGN_TRAIT)
  ...
  it('her adds are real, registered Rotcap Hollow roster members', () => {
    expect(ROT_SOVEREIGN_ADDS.length).toBeGreaterThan(0)
    for (const add of ROT_SOVEREIGN_ADDS) {
      expect(ALL_CREATURES).toContainEqual(add)
    }
  })
})
