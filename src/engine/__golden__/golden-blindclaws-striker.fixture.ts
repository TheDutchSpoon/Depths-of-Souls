...
// `ambush-strike` script (data/scripts.ts) only attacks when it can act before its target,
// holding position (Defends) otherwise -- the first real roster use of `acted-before-target`
// (Slice C). Real shipped content throughout (data/species/glimmerdark.ts's real
...
//     is FALSE -> ambush-strike's rule 1 (Attack) is skipped, rule 2 (Defend) fires instead.
//
...
//   STRIKER's turn: now queue-index 0 vs TARGET's index 1 -> acted-before-target is TRUE ->
//     ambush-strike's rule 1 fires: Attack. off = Attack(24, no stat-mods) x spellPower(1.0) +
//     crossStatBonus (Killing Instinct: fromStat speed, percentPerRank 0.2, appliesTo attack ->
//     0.2 x effective Speed(8) = 1.6), added post-spellPower per CONVENTIONS -> 25.6; vs def 0:
//     core 25.6, chip 0.256 -> raw 25.856 -> final 25. TARGET 82-25=57, survives.
...
// Round 1: TARGET, SETTER, STRIKER (Defends). Round 2: STRIKER again (now first, Attacks).
...
export const expectedEvents: CombatEvent[] = [
  ...
  // ambush-strike's rule 1 (acted-before-target) is false this round -- rule 2 (Defend) fires.
  { type: 'Defended', creatureId: STRIKER },
  { type: 'TurnEnded', creatureId: STRIKER },
  { type: 'RoundStarted', round: 2 },
  { type: 'TurnStarted', creatureId: STRIKER },
  // Now first in queue (grant-act-first, still active) -- acted-before-target is true.
