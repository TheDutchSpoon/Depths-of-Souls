...
src/engine/
  ...
  resolution.ts        EXTEND  executeResponse grows revive/grant-action-state/scoped-suppress;
                             ...
                             fireHook gains the four on-[action] call sites
...
|---|---|---|---|
| `on-attack`/`on-cast`/`on-defend`/`on-provoke` | Hook (+4, → 17) | B | Fires once, right after the action's intent event (`AttackDeclared`/`SpellCast`/`Defended`/`Provoked`), before that action's damage/consequences. Never re-fires from a response's own attack/cast (no cascade). |
| `revive` | Response (+1, → 7) | B | Target must be dead. Sets `alive: true`, `currentHp = round(baselineMaxHp * pct)`, and **resets `activeEffects` to a fresh instantiation of `innateTraitIds`** (death-reset: no ramp preserved). No special turn-queue handling needed — see Assumption 6. |
| `grant-action-state` | Response (+1, → 8) | B | Sets `defending` and/or `provoking` `true` on target. Reuses the existing flags/math (Defend's ×1.5/×0.65, Provoke's redirect) verbatim. |
...
| Acted-before-target condition | New `Condition` member | C | `{ kind: 'acted-before-target' }` — true iff, in the frozen round's turn queue, the acting creature's queue index is lower than its (selector-resolved) target's. Self-scoped like all triggered/scripting conditions; needs the target *pre-selected* to evaluate — see Assumption 10 on where in the pipeline this runs for scripting use. |
| Targeting-override pipeline | Targeting.ts restructure | C | Generalizes `resolveOffensiveTarget` into an ordered chain: **(1)** Tunnel Vision exemption (if attacker has it, skip straight to normal resolution, ignoring enemy Provoke) → **(2)** Confusion (if attacker is confused, 50% seeded roll to redirect the *entire already-chosen action* at its own side instead) → **(3)** Provoke (existing). See Assumption 11 for the Confusion/Provoke interaction order. |
| Adjacency + Splashing/Annihilate | Targeting helper + executor change | C | `adjacentLivingTargets(target, side)`: neighbors of `target`'s position **within the alive-filtered, slot-ordered list of its side** (not raw slot index — Assumption 12). Splashing: after a single-target Attack/Cast's main hit resolves, if the attacker carries the Splashing passive, **recompute the full damage formula** against each adjacent living enemy (not a copy of the main hit's number — Assumption 13) and apply as its own `DamageDealt`. Annihilate upgrades the target set from "adjacent" to "all other living enemies." |
...
| Consume-stacks response | Response (+1, → 9) | D | `{ kind: 'consume-stacks', statusId, effect: EffectResponse }`: reads the target's current stack count for `statusId`, removes the status entirely (0 stacks), then executes `effect` with the read count available as its magnitude source (`magnitudeSource: { kind: 'consumed-stacks' }`). |
| Cheat-death | New passive EffectDef category + resolver hook point | D | `{ category: 'cheat-death', chancePercent }`. Checked inside `applyDamageAndEmit`, at the instant HP would reach 0, **before** `CreatureDied`/`on-death` fire: one seeded RNG roll; on success, `currentHp = 1` instead of 0, no death events, resolution continues as if the hit landed for `finalDamage − 1`... — see Assumption 14 for the exact HP-left semantics. |
...
**Response vocabulary after Phase 4: nine** (`deal-damage`, `heal`, `apply-status`,
`apply-stat-modifier`, `suppress-action`, `revive`, `grant-action-state`, `consume-stacks`,
and `consume-stacks`'s wrapped inner `effect` reuses the other eight — so genuinely eight
top-level kinds + one wrapper). This is *not* "hold the line at six" from species-locked.md —
...
- **`generation.ts`**:
  ...
  - `generateFloor(floor, biomeData, runRng): { enemyParty: Creature[] }[]` (one entry per
    ...
    a species from the biome's spawn pool (uniform for now — **ASSUMPTION 5**: species-pool
    weighting, if any, is parked; v1 draws species uniformly, then a creature *within* the
    species by the declared rarity weights), draw a level from `enemyLevelRange(floor)`, call
...
- **`effect-types.ts`**: `Hook` grows to 17 (`on-attack`, `on-cast`, `on-defend`, `on-provoke`
  ...
  exclusive with `offStat`/`flatAmount` — **ASSUMPTION 6a**: if both are somehow set, throw a
...
- **`resolution.ts`**:
  - The four `on-[action]` hooks fire from `resolveTurn`'s action-execution branch, immediately
    after emitting `AttackDeclared`/`SpellCast`/`Defended`/`Provoked` and before that action's
    own damage/consequence resolution — a new, tiny, uniform call site per action kind. A
    response fired from one of these hooks that itself deals damage goes through the *normal*
    `executeResponse` → `applyDamageAndEmit` path and does **not** re-fire the originating
    `on-[action]` hook (it's a different hook context entirely — no cascade risk, no
    loop-guard interaction needed, confirmed against the existing cascade model).
  - `executeResponse` grows `revive` (see vocabulary table; **ASSUMPTION 6b**: `revive`'s
  ...
  - `suppress-action`'s scope: the interpreter's validity check (in `interpreter.ts`, not
    ...
    Wait are never suppressible in v1 (no content needs it — **ASSUMPTION 7**, flag if this
...
- **Tests**: unit coverage per new response kind and per formula extension (armor-pen at 0%/
  ...
  and nothing else); new goldens: `golden-on-action-hooks` (a fixture trait on each of the four
  new hooks, hand-derived), `golden-revive` (Unicorn-shaped: a dead ally comes back mid-fight
  at the death-reset baseline), `golden-armor-penetration`, `golden-cross-stat-contribution`,
...
- **`turn-order.ts`**: `buildTurnQueue` partitions the alive-filtered combatant list into three
  ...
  — confirmed by species-locked.md as "same tool, opposite pole." **ASSUMPTION 8**: Web's
...
- **`conditions.ts` / `scripting-types.ts`**: `AlreadyActedCondition` — wait, named
  `acted-before-target` per the design doc — `{ kind: 'acted-before-target' }`. **ASSUMPTION 9**:
...
- **`targeting.ts`**: restructure `resolveOffensiveTarget` into the three-step override
  pipeline from the vocabulary table. **ASSUMPTION 10 (Confusion/Provoke order)**: Confusion is
  ...
  decided. **ASSUMPTION 11**: Confusion's AOE case ("flip whole AOE to allies?", also flagged
  ...
  - `adjacentLivingTargets(target, partyOfTarget)`: neighbors in the **alive-filtered, slot-
    ordered** list (**ASSUMPTION 12**, resolving species-locked.md's flagged slot-vs-living
...
- **`resolution.ts` / executors**: Splashing consulted from `executeAttack`/
  ...
  Defence/affinity (**ASSUMPTION 13** — "100% of their damage" reads as "the same formula
...
- **`effect-types.ts` / `effects.ts`**: a `magnitudeSource` union usable wherever a
  ...
  'enemies-with-status' | 'dead-allies' | 'self-defend-count' }`. **ASSUMPTION 14**: adding this
  ...
  - `self-defend-count` needs a **new per-creature counter**, not derivable from existing state.
    **ASSUMPTION 15**: add `Creature.defendCount: number` (increment in `executeDefend`,
...
- **Consume-stacks response**: `{ kind: 'consume-stacks', statusId, effect: EffectResponse }` —
  ...
  16**: emits `StatusExpired` for the consumed status, since it's genuinely gone, not merely
...
- **Cheat-death**: `{ category: 'cheat-death', chancePercent }` passive, checked inside
  `applyDamageAndEmit` at the moment post-clamp HP would be `≤ 0`. **ASSUMPTION 17** (HP-left
...
- **`data/specializations.ts`**: `Specialization { id, name, starterCreatureId, perks:
  ...
  number) => EffectDef[]) }` — **ASSUMPTION 18**: leveled perks (e.g. Might: 50 levels, +1%
...
- **Perk execution model** — **ASSUMPTION 19 (the load-bearing new-plumbing decision this
...
- **Response reuse check (grounding, not new work)**: every Phase-4-functional perk in all
  ...
  file's own "New mechanics this spec introduces" section (Echo/Flurry → an
  extra-action-on-cast/attack response — **ASSUMPTION 20**: modeled as a `1 + N` execute-the-
  action-again loop **inside** the action executor when the actor carries the matching passive,
  *not* a triggered response off `on-cast`/`on-attack` — because "cast/attack an additional
  time" must re-run the *chosen* action with its *original* target/slot, which a hook-context
  response can't cleanly re-derive; this is the one perk-family mechanism that isn't a pure
  reuse of an existing response and needs its own small executor branch, called out explicitly
  since it's new resolver logic, not data).
- **`data/species/starters.ts`**: the three starter creatures (Wit Sorcerer starter with its
  ...
  on-attack-strike-again trait, high-Defence Shieldbarer starter with its
  on-provoke→team+35%-Defence trait via `grant-action-state`... **ASSUMPTION 21**: "team gains
  ...
  at `{ kind: 'random-dead-ally' }`, per-hit so a multi-attack effect revives multiple times —
  confirmed safe against loop-safety since `revive` deals no damage, so it cannot itself
  re-trigger a damage-path hook).
- **Scripted-intro encounter**: **ASSUMPTION 22**: this is **not** a new engine mechanism — it's
...
- **Tests**: a loader test per spec asserting the 1000-point invariant and that every perk's
  ...
  effect lists — Assumption 23: an equipment-gated perk with nothing to key off yet is
  ...
  starter's signature trait; a golden for the extra-action perk family (Flurry/Echo); a
...
- **`store.ts`** (Zustand, **in-memory only** — no persistence middleware): per CONVENTIONS'
  ...
  - `collection: Map<CreatureId, Instance[]>` and `activeParty: (InstanceId | null)[6]` —
    **ASSUMPTION 24**: an `Instance` gets its own branded `InstanceId` (distinct from the
  ...
  - `currencies: { essence, ore, bricks, lifeforce }` — **ASSUMPTION 25**: tracked now (floors
  ...
  - `runRng: SeededRng` **state** — CONVENTIONS: "the persistent run RNG stream." A store
    ...
    plain data" — **ASSUMPTION 26**: store the RNG's *seed* + an *advance counter*, not the
  ...
  - **Spec swap**: `setSpec(specId)` — refunds all spent points (perk points are `derived`, so
    ...
    collection if the player doesn't already own an instance of it (**ASSUMPTION 27**: swapping
...
- **H1 — The Overgrowth** (floors 1–30... wait, floor 1–10 per its own biome-decade; see
  Assumption 28): Spiders, Swarmhive, Treants, Pollinators, Snapjaws, Lullpollen; **Web**,
...
- **H3 — Rotcap Hollow**: Sporecloud, Rotfeeders, Myconet, Necromoss, Hollowkin, Sporch;
  **Spore** (DoT + spread-on-death — **ASSUMPTION 29**: spread-on-death is a trait-authored
...
**ASSUMPTION 28**: "floors 1–30" in ROADMAP/GAME_DESIGN's own phrasing means biome 1 = floors
...
5. Enemy species-within-biome draw is uniform in v1; creature-within-species uses the declared
   rarity weights.
6a. `deal-damage`'s `scalingStat` and `offStat`/`flatAmount` are mutually exclusive; setting
    both is a resolver-invariant error.
6b. `revive` needs a new `{ kind: 'random-dead-ally' }` `ResponseTarget` (existing selectors are
    alive-only).
7. Suppress-action scoping only ever gates Attack/Cast in v1 content — Defend/Provoke/Wait are
   never suppressible (revisit if future content needs it).
8. Web's break-free roll is a self-shortening status (mechanism TBD at implementation — flagged
   as the one bespoke piece in Slice C, needs its own small design pass, not a reuse).
9. A creature with both act-first and act-last active resolves to act-first (first wins).
10. `acted-before-target` resolves its own rule's target selector first (existence+selection,
...
11. Confusion is checked **before** Provoke in the targeting-override pipeline; its roll always
    happens for a confused creature's harmful action regardless of enemy-side provoke state.
12. Confusion's AOE case retargets the **whole action** with one roll (not a per-target coin
    flip).
13. Adjacency is computed over the **alive-filtered, slot-ordered** list of a side, not raw
    slot index (degrades gracefully as a side thins).
14. Splashing **recomputes** the full damage formula per adjacent target (own Defence/affinity
    apply) rather than copying the main hit's number.
15. `magnitudeSource` is an additive optional field alongside every existing flat
    `factor`/`flatAmount`/`amountPerStack`/`magnitude` — zero migration of Phase 1–3 content.
16. `Creature.defendCount: number` is a new dedicated field (cumulative all fight, never reset),
    not folded into `activeEffects`.
17. `consume-stacks` emits `StatusExpired` for the consumed status (it's genuinely gone).
18. Cheat-death sets `currentHp = 1` exactly (not a derived value) and only rolls when the hit
    would otherwise be lethal.
19. Leveled perks' effects are a function of purchased level, not a fixed definition; a
    load-time validator enforces `Σ(maxLevel × costPerLevel) === 1000` per spec.
20. Perks are **player-level** effects passed into `createCombat` as a new
...
21. Extra-action perks (Flurry/Echo) are a small dedicated executor branch (re-run the chosen
    action against its original target/slot), not a triggered response — the one perk
    mechanism that isn't a pure reuse of existing vocabulary.
