...
SpeciesCreature.equippedSpells); G: 492/492 tests, post-review-amendment (currency banking →
per-kill; + a wipe-with-mid-fight-kill regression test and a perk→combat plumbing test — count
re-stamps on the amendment's merge); lint/format/build green throughout). Built per
...
- `rewards.ts` — pure, independently-testable run-layer reward/lookup helpers, mirroring the
  ...
  - `currencyDropForKill(floor)` — a flat, floor-scaled placeholder (parked balance, GAME_DESIGN
    §13), granted **per kill** (per `CreatureDied` enemy), the same treatment as soul% and XP. The
    amount is **creature-independent** — a flat per-kill value, not keyed to species or rarity
    (souls carry the rarity skew; currency does not). This matches GAME_DESIGN §7 and CONVENTIONS,
    which both group soul%/XP/currency as banking per kill-event, never held pending the fight's
    outcome, kept on wipe. (An earlier draft of this slice granted currency per *fight won*, on a
    misreading that applied GAME_DESIGN §4's "independent of which specific creature was defeated"
    clause — which is scoped to *recipe* drops — to currency; corrected in review. Per-kill and
    creature-independence are not in tension: a flat per-kill amount satisfies both.)
...
- `store.ts` — `createGameStore(overrides): UseBoundStore<...>` (Zustand `create`, no
  ...
  - **`descend(floor)`** — the integration action. Bounded to `1..deepestFloor+1` (own
    ...
    guesswork; an unexpected lookup miss on a generated enemy throws rather than silently dropping
    the reward) and looked up via `findStaticCreature`, and soul%/XP/currency are banked
    immediately (GAME_DESIGN §7 / CONVENTIONS: "never held pending fight outcome") — **before**
    checking whether that fight was even won, so an enemy killed in a fight the party then wipes on
    still banks all three. The loop stops (without attempting the floor's remaining fights) at the
    first non-`'win'` result; `deepestFloor` only advances on a full clear. Returns a `FloorOutcome`
...
Post-review-amendment adds: a **wipe-with-a-mid-fight-kill** regression test — on a floor with
`enemyPartySize ≥ 2`, an enemy dies and the party then wipes in the same fight, asserting soul%
**and** XP **and currency** are all banked for that kill (the case that distinguishes per-kill from
per-fight-won banking, which the floor-1 tests can't reach since `enemyPartySize(1) = 1`); and a
**perk→combat plumbing** test that injects a non-empty `perkSpend` with a materially-effecting
fixture perk and asserts the resolved effects reach `createCombat` (a fight lost at empty spend but
won with the perk applied), so the `partyWidePlayerEffects` path isn't left unexercised.
...
- **Currency banking granularity** — banks **per kill**, at a flat, creature-independent amount,
  kept on wipe (GAME_DESIGN §7 / CONVENTIONS), consistent with soul% and XP. An earlier draft
  granted it per *fight won*; corrected in review. The apparent GAME_DESIGN §4-vs-CONVENTIONS
  tension was a misreading — §4's "independent of which specific creature" clause is about *recipe*
  drops, and a flat per-kill amount satisfies creature-independence without moving off per-kill
  granularity. H1+ content should not assume currency is creature-keyed.
